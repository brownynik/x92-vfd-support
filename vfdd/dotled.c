...
int dotled_test_card(AppClass *data, void *user_data );
#endif
...
#ifdef X92_COMPATIBLE
   // override param "dotleds"->"grid_map" from .conf file
   // param "target" for select word in overlay.
   int target_index;
   n = json_root_get_item_int(node, "target",  &target_index);
   if ( ! n ){
      msg_error( "dotled target not defined" );
   } else led->target = &target[target_index];
#else
   led->target = target;
#endif
...
static TestDotval test_fun_tbl[] = {
   { "net", dotled_test_net              },
   { "hdmi", dotled_test_hdmi            },
   { "colon", dotled_test_colon          },
   { "bluetooth", dotled_test_bluetooth  },
   { "usb", dotled_test_usb              },
#ifdef X92_COMPATIBLE
   { "card", dotled_test_card            },
#endif
   { "alarm", dotled_test_alarm          },
   { NULL, NULL                          },
};
